...
class ObjectDetectionProcessor:
    ...
    async def process_image(self, image_path, filename):
        loop = asyncio.get_event_loop()
        image = await loop.run_in_executor(None, read_image, image_path)
        ...
        result = await loop.run_in_executor(None, get_sliced_prediction, image_path, self.detection_model, slice_height, slice_width, 0.3, 0.3, True)
        ...
        initial_image = await loop.run_in_executor(None, cv2.imread, image_path)
        initial_image_copy = initial_image.copy()
        for box, pred in zip(boxes, predictions):
            x1, y1, x2, y2 = box
            class_id = pred.category.id
            score = pred.score.value
            await loop.run_in_executor(None, cv2.rectangle, initial_image_copy, (x1, y1), (x2, y2), (0, 255, 0), 2)
            await loop.run_in_executor(None, cv2.putText, initial_image_copy, f'{class_id} {score:.2f}', (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (36, 255, 12), 2)
        ...
        for i in range(len(boxes)):
            ...
            for j in range(i + 1, len(boxes)):
                ...
                if self.has_intersection(current_box, boxes[j]):
                    ...
                    crop_result = await loop.run_in_executor(None, get_prediction, crop_img, self.detection_model)
        ...
        await self.save_yolo_format(final_predictions, width, height, base)

        return final_predictions

    async def save_yolo_format(self, predictions, img_width, img_height, base_filename):
        yolo_output_path = os.path.join(self.output_dir, f'{base_filename}.txt')
        async with aiofiles.open(yolo_output_path, 'w') as file:
            for pred in predictions:
                ...
                await file.write(f"{class_id} {x_center:.6f} {y_center:.6f} {width:.6f} {height:.6f}\n")
        ...
        with ThreadPoolExecutor() as executor:
            ...
            for filename in os.listdir(self.input_dir):
                if filename.lower().endswith(('.png', '.jpg', '.jpeg')):
                    ...
                    tasks.append(loop.run_in_executor(executor, self.process_image, image_path, filename))
